          ...
          <div className="grid md:grid-cols-3 gap-8">
            {skillCategories.map((category, index) => (
              <motion.div
                ...
                className="relative"
              >
                <div className="bg-slate-900/50 rounded-2xl p-8 backdrop-blur-sm border border-slate-700 hover:border-purple-500/50 transition-all duration-500 h-full">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {category.skills.map((skill, idx) => (
                      <motion.span
                        ...
                        viewport={{ once: true }}
                        className="bg-slate-700 text-gray-300 px-3 py-1 rounded-full text-sm hover:bg-slate-600 transition-colors cursor-default"
                      >
                        {skill}
                      </motion.span>
                    ))}
